...
class BTreeInnerNode<TKey extends Comparable<TKey>> extends BTreeNode<TKey> implements Serializable{
	...
	{
		String s  = ("{" + Arrays.toString(this.children) + "}" + "\n");
		for(int i =0;i<this.children.length;i++)
		{
			if(this.children[i] instanceof BTreeInnerNode)
				s += ((BTreeInnerNode) this.children[i]).printTree();
			else if(this.children[i] != null)
				System.out.println(Arrays.toString(((BTreeLeafNode)this.children[i]).values));
		}
		return s;
	}
